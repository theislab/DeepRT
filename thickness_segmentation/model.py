...
def get_unet(params):
    ...
    c5 = conv2d_block(p4, n_filters=params.n_filters * 16, kernel_size=3, batchnorm=params.batchnorm)

    # expansive path
    u9 = Conv2DTranspose(params.n_filters * 8, (3, 3), strides=(2, 2), padding='same')(c5)
...
def get_deep_unet_v2(params):
    ...
    c6 = conv2d_block(p5, n_filters=params.n_filters * 8, kernel_size=3, batchnorm=params.batchnorm)
    p6 = MaxPooling2D(pool_size=(2, 2))(c6)
    p6 = Dropout(params.dropout)(p6, training=params.is_training)

    c7 = conv2d_block(p6, n_filters=params.n_filters * 16, kernel_size=3, batchnorm=params.batchnorm)

    # expansive path
    u8 = Conv2DTranspose(params.n_filters * 8, (3, 3), strides=(2, 2), padding='same')(c7)
    u8 = concatenate([u8, c6])
    u8 = Dropout(params.dropout)(u8, training=params.is_training)
    c8 = conv2d_block(u8, n_filters=params.n_filters * 8, kernel_size=3, batchnorm=params.batchnorm)

    u9 = Conv2DTranspose(params.n_filters * 8, (3, 3), strides=(2, 2), padding='same')(c8)
    u9 = concatenate([u9, c5])
    u9 = Dropout(params.dropout)(u9, training=params.is_training)
    c9 = conv2d_block(u9, n_filters=params.n_filters * 8, kernel_size=3, batchnorm=params.batchnorm)

    u10 = Conv2DTranspose(params.n_filters * 8, (3, 3), strides=(2, 2), padding='same')(c9)
    u10 = concatenate([u10, c4])
    u10 = Dropout(params.dropout)(u10, training=params.is_training)
    c10 = conv2d_block(u10, n_filters=params.n_filters * 8, kernel_size=3, batchnorm=params.batchnorm)

    u11 = Conv2DTranspose(params.n_filters * 4, (3, 3), strides=(2, 2), padding='same')(c10)
    u11 = concatenate([u11, c3])
    u11 = Dropout(params.dropout)(u11, training=params.is_training)
    c11 = conv2d_block(u11, n_filters=params.n_filters * 4, kernel_size=3, batchnorm=params.batchnorm)

    u12 = Conv2DTranspose(params.n_filters * 2, (3, 3), strides=(2, 2), padding='same')(c11)
    u12 = concatenate([u12, c2])
    u12 = Dropout(params.dropout)(u12, training=params.is_training)
    c12 = conv2d_block(u12, n_filters=params.n_filters * 2, kernel_size=3, batchnorm=params.batchnorm)

    u13 = Conv2DTranspose(params.n_filters * 1, (3, 3), strides=(2, 2), padding='same')(c12)
    u13 = concatenate([u13, c1])
    u13 = Dropout(params.dropout)(u13, training=params.is_training)
    c13 = conv2d_block(u13, n_filters=params.n_filters * 1, kernel_size=3, batchnorm=params.batchnorm)
