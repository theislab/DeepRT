...
logging_dir = '/home/olle/PycharmProjects/segmentation_OCT/import_env/OCT_segmentation/logging_u_net_no_preprocc_dice/'
import scipy.misc
...
def unpadding_with_zeros(im, orig_shape, new_shape):
    '''
    :param im:
    :param orig_shape:
    :param new_shape:
...
for image_dir in images_left_to_complete:
    try:
        completed_images.append(image_dir)
        print(image_dir)
        #split up the path and change to segmenation map path
        im_name = image_dir.split("/")[-1]
        im_main_dir = "/".join(image_dir.split("/")[0:-1])
        #measure time to load data
        starting_data_loading = dt.datetime.now()
        im,new_shape, orig_shape = id.get_clinic_data_hardrive(image_dir, img_width, img_height)
        ...
        im = im.reshape(160,400)
        #print(im.shape, new_shape, orig_shape)
        new_weighted_threshold = 0.83#new_weighted_threshold
        feed_dict_train = {X: im.reshape(1, img_height, img_width, 1)}
        ...
        pred = pred.reshape(160,400).astype(np.int32)
        im_resized = np.asarray(Image.fromarray(pred).resize([new_shape[1],new_shape[0]]))
        #print(im_resized.shape)
        result = unpadding_with_zeros(im_resized, orig_shape, new_shape)
        #print(result.shape)
        starting_saving = dt.datetime.now()
        scipy.misc.imsave(im_main_dir + "/seg_"+im_name, result)
        k += 1
        ending_saving = dt.datetime.now()
        saving_time.append(abs((ending_saving.microsecond-starting_saving.microsecond))/1e6)
        # delete the image from the list as processed
        images_left_to_complete.remove(image_dir)
        ...
        print(len(images_left_to_complete))
    except:
